...
def test_db_session():
    ...
    with SQLModelSession(engine) as session:
        ...
        default_group = session.query(PublicationGroup).filter_by(
            name=DEFAULT_GROUP_NAME
        ).first()
        if default_group is None:
            session.add(PublicationGroup(name=DEFAULT_GROUP_NAME))
            session.commit()
        yield session
...
def test_group(test_db_session):
    ...
    if not group:
        group = PublicationGroup(
            name="DEFAULT-PUBLICATION-GROUP", accepting_submissions=True
        )
...
def valid_jwt_token(test_user_session):
    ...
    token = sign_jwt(
        {
            "email": test_user_session.email,
            "id": test_user_session.account_id,
            "session": test_user_session.client_token,
        },
...
@pytest.fixture
def test_user_session(test_account, test_session_manager):
    ...
    session.client_token = "test_client_token_123"
